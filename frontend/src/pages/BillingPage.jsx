...
const BillingPage = () => {
  ...
  const fetchInvoices = () => {
    billingAPI
      .listInvoices({ status: statusFilter || undefined })
  ...
  const totalRevenue = invoices.reduce(
    (s, inv) => s + parseFloat(inv.paid_amount || 0),
    0,
  );
  const totalPending = invoices.reduce(
    (s, inv) =>
      s + parseFloat(inv.total_amount || 0) - parseFloat(inv.paid_amount || 0),
    0,
  );
              ...
              {invoices.length > 0 ? (
                invoices.map((inv) => (
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`badge badge-${inv.status === "paid" ? "success" : inv.status === "overdue" ? "danger" : inv.status === "partial" ? "warning" : "info"}`}
                      >
                        {inv.status}
                      </span>
                    </td>
            ...
            <div className="card">
              ...
              >
                ...
                >
                  <span>Total</span>
                  <span>
                    ₹{parseFloat(selected.total_amount).toLocaleString()}
                  </span>
                </div>
                <div
                  style={{
                ...
                >
                  <span>Paid</span>
                  <span>
                    ₹{parseFloat(selected.paid_amount).toLocaleString()}
                  </span>
                </div>
              </div>
            ...
            <form onSubmit={handlePayment}>
              <div className="form-group">
                <label>
                  Amount (Balance: ₹
                  {(
                    parseFloat(selected.total_amount) -
                    parseFloat(selected.paid_amount)
                  ).toLocaleString()}
                  )
                </label>
                <input
